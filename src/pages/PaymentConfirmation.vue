      ...
      <div class="col-12 col-md-12 q-pl-md-md">
        <q-card flat class="tf-rounded-5 q-pa-xl" v-if="!loading">
          ...
          <div class="items-center text-center column" v-if="status_info">
            ...
            <h4 class="text-h4 tf-max-w-600 text-grey-9 tf-font-anton">
              {{ status_info.message }}
            </h4>
            <h6 class="text-h6 tf-max-w-600 text-grey-7 tf-font-hero">
              {{ status_info.info }}
            </h6>
...
export default {
  ...
  data() {
    return {
      successRoute: { route: { name: "user.transactions" }, label: "View Transactions" },
      loading: false,
      value: 0,
